...
with outer[1]:
    ...
    with st.container(border=True):
        ...
        fc = st.columns(7)
        fc[0].page_link("views/weight.py", label="Weight & BOM", icon="📦",
                        use_container_width=True)
        fc[1].page_link("views/wing_loading.py", label="Wing & Power",
                        icon="🪁", use_container_width=True)
        fc[2].page_link("views/trim_thrust.py", label="Trim & Thrust",
                        icon="🚁", use_container_width=True)
        fc[3].page_link("views/trim_alpha.py", label="α-Trim", icon="📐",
                        use_container_width=True)
        fc[4].page_link("views/moment.py", label="Moment", icon="⚖️",
                        use_container_width=True)
        fc[5].page_link("views/propeller.py", label="Propeller", icon="🌀",
                        use_container_width=True)
        fc[6].page_link("views/battery.py", label="Battery", icon="🔋",
                        use_container_width=True)
